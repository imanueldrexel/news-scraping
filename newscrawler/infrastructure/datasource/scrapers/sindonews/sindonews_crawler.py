...
from newscrawler.core.utils.utils import get_last_crawling_time, preprocess_text, \
...
class SindonewsCrawler(Crawler):
    ...
    def get_news_data(self, web_url: str) -> NewsInformationDTO:
        ...
        set_last_crawling_time(last_crawling_time, self.main_path, self.website_name)
    ...
    def _get_text(soup):
        layer = None
        if layer is None: layer = soup.find("div", attrs={'itemprop': ['articleBody'], "class": ["caption", None]})
        if layer is None: layer = soup.find("div",
                                            attrs={"class": ["article", "desc-artikel-detail"], "itemprop": None})
        if layer is None: layer = soup.find("section", attrs={"class": "article col-md-11"})
        ...
        for x in layer.contents:
            if type(x) == NavigableString:
                ...
                avoided_keyword = ["Baca Juga", "Baca juga", "Lihat Juga", "Lihat juga", "Penulis:",
                                   "Penulis :", "Reporter:", "Simak berita lainnya seputar",
