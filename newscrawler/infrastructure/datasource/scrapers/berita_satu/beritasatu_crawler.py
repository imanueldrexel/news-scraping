...
class BeritaSatuCrawler(Crawler):
    ...
    def _get_whole_text(soup) -> List[str]:
        article_layer = soup.find_all("div", {"class": "story"})
        for article in article_layer:
            attributes = article.attrs["class"]
            if len(attributes) == 1:  # only take class = 'story'
                while article.find_all("div"):
                    article.div.decompose()
                while article.find_all("script"):
                    article.script.decompose()
                sentences = article.find_all("p", {"class": None})
                texts = []
