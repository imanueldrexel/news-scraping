...
from newscrawler.core.utils.utils import get_last_crawling_time, set_last_crawling_time, \
    preprocess_text
from newscrawler.domain.entities.extraction.news_data import NewsData
...
class KontanCrawler(Crawler):
    ...
    def _get_branches(soup) -> Dict[str, str]:
        ...
        for sitemap in sitemaps:
            avoided_keyword = ["insight", "edsus", "exportexpert", "kilaskementerian", "kilaskorporasi", "pialaeropa"]
            ...
            if link:
                ...
                if "/news/" in link:
                    ...
                    if branch_name == "www":
                        branch_name = re.sub(r"(https://www.kontan.co.id/)(.*)(/news/sitemap.xml)", r"\2", link)
    ...
    def _get_whole_text(soup):
        layer = soup.find("div", attrs={'itemprop': ['articleBody'], "class": ["tmpt-desk-kon", "ctn", None]})
        if layer:
            sentences = layer.find_all("p")
            texts = []
            for sentence in sentences:
                avoided_keyword = ["Baca Juga", "Baca juga", "Lihat Juga", "Lihat juga", "Selanjutnya", "Penulis:",
                                   "Penulis :", "Reporter:", ]
                continue_sentence = 0
                for keyword in avoided_keyword:
