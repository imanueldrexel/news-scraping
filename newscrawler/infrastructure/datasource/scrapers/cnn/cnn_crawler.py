        ...
        for sitemap in sitemaps:
            ...
            if link:
                ...
                if "sitemap_news" in link:
                    branch_name = re.sub(r"(https://www.cnnindonesia.com/)(.*)(/)(.*)(/*)", r"\2", link)
    ...
    def _scrape(self, branch_link, branch_name, last_stamped_crawling=None) -> Tuple[date, List]:
        ...
        if soup is None:
            ...
            for idx, url in enumerate(soup.find_all("url")):
                ...
                timestamp_string, timestamp_datetime = self._get_timestamp(news_soup=url,
                                                                           date_time_reader=self.date_time_reader)
                time_posted, delta, delta_in_second = self._get_delta_and_delta_in_second(timestamp=timestamp_string,
