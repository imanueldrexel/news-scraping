...
from newscrawler.core.utils.utils import get_last_crawling_time, preprocess_text, set_last_crawling_time
...
class PikiranRakyatCrawler(Crawler):
    ...
    def _scrape(self, soup) -> Tuple[Dict[str, date], List]:
        ...
        for idx, url in enumerate(soup.find_all("url")):
            ...
            last_stamped_crawling = self.last_crawling_time.get(branch_name, self.default_time)
    ...
    def _get_branch_name(text):
        branch_name = re.sub(r"(https://)(.*)(.pikiran-rakyat.com)(.*)(/pr.*)(.*)", r"\2\4", text)
        return branch_name
        ...
        if layer:
            ...
            for sentence in sentences:
                avoided_keyword = ["Baca Juga", "Baca juga", "Lihat Juga", "Lihat juga", "Penulis:",
                                   "Penulis :", "Editor:", "Sumber: ", "Reporter:", "Simak berita lainnya seputar",
