...
from newscrawler.core.utils.utils import get_last_crawling_time, \
    set_last_crawling_time, preprocess_text
import logging
...
class LiputanEnamCrawler(Crawler):
    ...
    def _get_whole_text(self, soup):
        pages = soup.find_all("div", attrs={"class": "article-content-body__item-content"})
        full_text = []
        for page in pages:
    ...
    def _get_branch_name(text):
        ...
        if branch_name == "www":
            branch_name = re.sub(r"(https://www.liputan6.com/)(.*)(/read)(.*)", r"\2", text)
